...
function ratingTemplate(rating) {
    const filledStars = '<span aria-hidden="true" class="icon-star">⭐</span>'.repeat(rating);
    const emptyStars = '<span aria-hidden="true" class="icon-star-empty">☆</span>'.repeat(5 - rating);
...
function searchHandler(e) {
    ...
    const recipeContainer = document.querySelector('main');
    recipeContainer.innerHTML = filteredRecipes.map(recipeTemplate).join('');
}
